#!/bin/bash
# Drive Vivado to synthesise and build the Arcade_Galaga Basys3 bitstream.
#
# Usage: make_arcade_galaga_basys3_bitstream.sh {synth|bitstream}
#
...
#   basys3/arcade_galaga_basys3.runs/impl_1/arcade_galaga_basys3.bit
#
...
XPR="$ROOT/basys3/arcade_galaga_basys3.xpr"
...
WORK=/tmp/arcade_galaga_bitstream
mkdir -p "$WORK"
...
if [ "$MODE" = "synth" ]; then
    ...
    BIT="$ROOT/basys3/arcade_galaga_basys3.runs/impl_1/arcade_galaga_basys3.bit"
    if [ -f "$BIT" ]; then
        echo "Bitstream: $BIT"
    else
